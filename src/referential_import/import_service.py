...
class ReferentialImportService:
    ...
    def analyze(self, input_path: str | Path) -> dict[str, Any]:
        ...
        for table_page in table_pages:
            ...
            if crit_lines:
                _, page_criteria, context = parse_competency_and_criteria_lines(crit_lines, context=context, page_number=table_page.page_number)
                criteria.extend(page_criteria)

            if not table_page.header_detected and table_page.columns.get("full_text"):
                fallback_pages = [cell.text for cell in table_page.columns["full_text"]]
                fallback_lines = iter_fallback_lines(fallback_pages)
                line_pairs = [(line.line_number, line.text) for line in fallback_lines]
                page_comps, page_criteria, context = parse_competency_and_criteria_lines(line_pairs, context=context, page_number=table_page.page_number)
                competencies.extend(page_comps)
                criteria.extend(page_criteria)
