...
def normalize_offer(raw_offer: dict[str, Any], *, model_skills: list[dict[str, Any]] | None = None) -> NormalizedOffer:
    ...
    rome_code = clean_text(_pick(raw_offer, 'romeCode', 'rome_code', 'code_rome'))
    rome_label = clean_text(_pick(raw_offer, 'romeLibelle', 'romeLabel', 'rome_label'))
    if not rome_code or not rome_label:
        rome = raw_offer.get('rome') or {}
        if isinstance(rome, dict):
            rome_code = rome_code or clean_text(_pick(rome, 'romeCode', 'code'))
            rome_label = rome_label or clean_text(_pick(rome, 'romeLibelle', 'libelle', 'label'))
